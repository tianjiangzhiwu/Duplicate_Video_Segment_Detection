...
"""
视频重复片段检测工具 - 一键运行
使用方法: python main.py [--auto-delete]
"""
...
from auto_video_editor import execute_auto_deletion
import json
import os
...
def main():
    # 检查命令行参数
    auto_delete_mode = '--auto-delete' in sys.argv or '--autodelete' in sys.argv
    ...
    if auto_delete_mode:
        print("\n启动自动删除模式...")
        execute_auto_deletion()
    else:
        # 询问用户是否执行自动删除
        response = input("\n是否执行自动删除重复片段功能？[Y/n]: ").strip().lower()
        if response in ('y', 'yes', '是', ''):
            execute_auto_deletion()
        else:
            print("跳过自动删除，您可以通过以下方式手动执行：")
            print("- 运行 python -c \"from auto_video_editor import execute_auto_deletion; execute_auto_deletion()\"")
            print("- 或运行 python main.py --auto-delete")
